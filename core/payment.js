...
class PaymentService {
    ...
    async handleWebhook(gateway, data) {
        try {
            ...
            switch (gateway) {
                case 'yookassa':
                    const yookassaResult = this.parseYooKassaWebhook(data);
                    paymentId = yookassaResult.paymentId;
                    gatewayPaymentId = yookassaResult.gatewayPaymentId;
                    amount = yookassaResult.amount;
                    status = yookassaResult.status;
                    break;
                case 'stripe':
                    const stripeResult = this.parseStripeWebhook(data);
                    paymentId = stripeResult.paymentId;
                    gatewayPaymentId = stripeResult.gatewayPaymentId;
                    amount = stripeResult.amount;
                    status = stripeResult.status;
                    break;
                case 'robokassa':
                    const robokassaResult = this.parseRobokassaWebhook(data);
                    paymentId = robokassaResult.paymentId;
                    gatewayPaymentId = robokassaResult.gatewayPaymentId;
                    amount = robokassaResult.amount;
                    status = robokassaResult.status;
                    break;
                default:
            ...
            if (paymentId) {
                if (status === 'success') {
                    await this.confirmPayment(paymentId, gatewayPaymentId, { webhook_data: data });
                } else {
                    await this.failPayment(paymentId, `Webhook status: ${status}`);
                }
    ...
    parseYooKassaWebhook(data) {
        try {
            ...
            return {
                paymentId: metadata.payment_id || object.payment_id,
                gatewayPaymentId: object.id || object.gateway_payment_id,
                amount: object.amount?.value || object.amount,
                status: object.status === 'succeeded' || object.status === 'waiting_for_capture' ? 'success' : object.status,
            };
        } catch (error) {
            logger.error('Error parsing YooKassa webhook:', error);
            return { paymentId: null, gatewayPaymentId: null, amount: null, status: 'failed' };
        }
    }

    parseStripeWebhook(data) {
        try {
            const object = data.data?.object || data;
            
            return {
                paymentId: object.metadata?.payment_id || object.payment_id,
                gatewayPaymentId: object.id || object.gateway_payment_id,
                amount: object.amount_total ? object.amount_total / 100 : object.amount,
                status: object.payment_status === 'paid' || object.status === 'complete' ? 'success' : object.status,
            };
        } catch (error) {
            logger.error('Error parsing Stripe webhook:', error);
            return { paymentId: null, gatewayPaymentId: null, amount: null, status: 'failed' };
        }
    }

    parseRobokassaWebhook(data) {
        try {
            ...
            return {
                paymentId: invId,
                gatewayPaymentId: `robokassa_${invId}`,
                amount: outSum,
                status: 'success',
            };
        } catch (error) {
            logger.error('Error parsing Robokassa webhook:', error);
            return { paymentId: null, gatewayPaymentId: null, amount: null, status: 'failed' };
        }
    }
